...
class CartoBoostRanker(BaseEstimator):
    ...
    Example:
        >>> ranker = CartoBoostRanker(n_estimators=8, max_depth=1, split_policy="axis_only")
    ...
    def load(cls, path: str | Path) -> CartoBoostRanker:
        ...
        envelope = decode_stable_model_artifact(payload, "ranker")
        inner = envelope["payload"]
        native_payload = inner.get("native_model")
        if not isinstance(native_payload, dict):
            raise ValueError("stable ranker artifact payload is missing native_model")
        with tempfile.TemporaryDirectory() as temp_dir:
            native_path = Path(temp_dir) / "native-ranker.json"
        ...
        estimator.group_col = inner.get("group_col")
