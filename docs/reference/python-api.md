...
| `NeuralPanelForecaster` | Rust-native neural panel forecaster with `n_lags`, `n_forecasts`, quantiles, trend, Fourier seasonality, event offsets, known-future regressors, lagged regressors, direct horizons, local/global component modes, and serializable metadata. |
| `LaneNeuralPanelForecaster` | Taxi lane wrapper for `series_id="origin:destination"` panels; records origin, destination, lane, directional graph-feature, and cold-lane fallback metadata while keeping `A:B` distinct from `B:A`; `predict_for_lanes(horizon, series_ids)` applies fitted-lane fallback for explicit cold lane ids. |
