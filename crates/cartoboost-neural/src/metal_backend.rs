...
use crate::backend::CsrDiffusionBackward;
use crate::backend::{BackendDispatchReport, BackendSelection};
...
pub(crate) fn csr_diffusion(
    ...
    const SOURCE: &str = r#"
        ...
        kernel void csr_diffusion(
            ...
            uint item = id / channels;
            uint row = item % nodes;
            uint batch = item / nodes;
            float sum = 0.0f;
    ...
    let (device, queue, pipeline) = metal_pipeline(SOURCE, "csr_diffusion")?;
    let buffers = (
        metal_input_buffer(&device, indptr),
        metal_input_buffer(&device, indices),
        metal_input_buffer(&device, weights),
        metal_input_buffer(&device, values),
        metal_output_buffer::<f32>(&device, values.len()),
        metal_input_buffer(&device, &[nodes]),
        metal_input_buffer(&device, &[channels_u32]),
    );
    metal_dispatch(
        &queue,
        &pipeline,
    ...
    Ok(metal_read(&buffers.4, values.len()))
}
...
pub(crate) fn csr_diffusion_backward(
    ...
    const SOURCE: &str = r#"
        #include <metal_stdlib>
        using namespace metal;
        kernel void csr_diffusion_backward(
            const device uint* indptr [[buffer(0)]],
            const device uint* indices [[buffer(1)]],
            const device float* weights [[buffer(2)]],
            const device float* values [[buffer(3)]],
            const device float* output_grad [[buffer(4)]],
            device atomic_float* input_grad [[buffer(5)]],
            device atomic_float* edge_grad [[buffer(6)]],
            constant uint& nodes [[buffer(7)]],
            ...
            uint row = item % nodes;
            uint batch = item / nodes;
            float gradient = output_grad[id];
            for (uint edge = indptr[row]; edge < indptr[row + 1]; ++edge) {
                uint input = (batch * nodes + indices[edge]) * channels + channel;
                atomic_fetch_add_explicit(&input_grad[input], weights[edge] * gradient, memory_order_relaxed);
                atomic_fetch_add_explicit(&edge_grad[edge], gradient * values[input], memory_order_relaxed);
            }
        }
    "#;
    let nodes = (indptr.len() - 1) as u32;
    let channels_u32 = channels as u32;
    let (device, queue, pipeline) = metal_pipeline(SOURCE, "csr_diffusion_backward")?;
    let buffers = (
        metal_input_buffer(&device, indptr),
        metal_input_buffer(&device, indices),
        metal_input_buffer(&device, weights),
        metal_input_buffer(&device, values),
        metal_input_buffer(&device, output_grad),
        metal_input_buffer(&device, &vec![0.0f32; values.len()]),
        metal_input_buffer(&device, &vec![0.0f32; weights.len()]),
        metal_input_buffer(&device, &[nodes]),
        metal_input_buffer(&device, &[channels_u32]),
    );
    metal_dispatch(
    ...
    Ok(CsrDiffusionBackward {
        input_grad: metal_read(&buffers.5, values.len()),
        edge_grad: metal_read(&buffers.6, weights.len()),
    })
}
...
pub(crate) fn csr_row_softmax(indptr: &[u32], logits: &[f32]) -> Result<Vec<f32>> {
    const SOURCE: &str = r#"
        #include <metal_stdlib>
        using namespace metal;
        kernel void csr_row_softmax(
            const device uint* indptr [[buffer(0)]],
            const device float* logits [[buffer(1)]],
            device float* output [[buffer(2)]],
            uint row [[thread_position_in_grid]]
        ) {
            uint start = indptr[row], end = indptr[row + 1];
            if (start == end) return;
            float maximum = logits[start];
            for (uint edge = start + 1; edge < end; ++edge) maximum = max(maximum, logits[edge]);
            float denominator = 0.0f;
            for (uint edge = start; edge < end; ++edge) denominator += exp(logits[edge] - maximum);
            for (uint edge = start; edge < end; ++edge) output[edge] = exp(logits[edge] - maximum) / denominator;
        }
    "#;
    let (device, queue, pipeline) = metal_pipeline(SOURCE, "csr_row_softmax")?;
    let buffers = (
        metal_input_buffer(&device, indptr),
        metal_input_buffer(&device, logits),
        metal_output_buffer::<f32>(&device, logits.len()),
    );
    metal_dispatch(
        &queue,
        &pipeline,
    ...
    Ok(metal_read(&buffers.2, logits.len()))
}
...
pub(crate) fn csr_row_softmax_backward(
    ...
    const SOURCE: &str = r#"
        #include <metal_stdlib>
        using namespace metal;
        kernel void csr_row_softmax_backward(
            const device uint* indptr [[buffer(0)]],
            const device float* weights [[buffer(1)]],
            const device float* output_grad [[buffer(2)]],
            device float* logits_grad [[buffer(3)]],
            uint row [[thread_position_in_grid]]
        ) {
            uint start = indptr[row], end = indptr[row + 1];
            float dot = 0.0f;
            for (uint edge = start; edge < end; ++edge) dot += weights[edge] * output_grad[edge];
            for (uint edge = start; edge < end; ++edge) logits_grad[edge] = weights[edge] * (output_grad[edge] - dot);
        }
    "#;
    let (device, queue, pipeline) = metal_pipeline(SOURCE, "csr_row_softmax_backward")?;
    let buffers = (
        metal_input_buffer(&device, indptr),
        metal_input_buffer(&device, weights),
        metal_input_buffer(&device, output_grad),
        metal_output_buffer::<f32>(&device, weights.len()),
    );
    metal_dispatch(
        &queue,
        &pipeline,
    ...
    Ok(metal_read(&buffers.3, weights.len()))
}
...
pub(crate) fn adamw(
    ...
    const SOURCE: &str = r#"
        ...
        kernel void adamw(
            ...
            device float* first [[buffer(1)]],
            device float* second [[buffer(2)]],
            const device float* gradients [[buffer(3)]],
            constant float& learning_rate [[buffer(4)]],
            constant float& weight_decay [[buffer(5)]],
            constant float& correction1 [[buffer(6)]],
            constant float& correction2 [[buffer(7)]],
            uint id [[thread_position_in_grid]]
        ) {
            float gradient = gradients[id];
            float m = 0.9f * first[id] + 0.1f * gradient;
            float v = 0.999f * second[id] + 0.001f * gradient * gradient;
            first[id] = m; second[id] = v;
            float update = (m / correction1) / (sqrt(v / correction2) + 1.0e-8f);
            parameters[id] -= learning_rate * (update + weight_decay * parameters[id]);
        }
    "#;
    let correction1 = 1.0 - 0.9f32.powi(step.min(i32::MAX as u64) as i32);
    let correction2 = 1.0 - 0.999f32.powi(step.min(i32::MAX as u64) as i32);
    let (device, queue, pipeline) = metal_pipeline(SOURCE, "adamw")?;
    let buffers = (
        metal_input_buffer(&device, parameters),
        metal_input_buffer(&device, first_moment),
        metal_input_buffer(&device, second_moment),
        metal_input_buffer(&device, gradients),
        metal_input_buffer(&device, &[learning_rate]),
        metal_input_buffer(&device, &[weight_decay]),
        metal_input_buffer(&device, &[correction1]),
        metal_input_buffer(&device, &[correction2]),
    );
    metal_dispatch(
        &queue,
        &pipeline,
    ...
    parameters.copy_from_slice(&metal_read(&buffers.0, parameters.len()));
    first_moment.copy_from_slice(&metal_read(&buffers.1, first_moment.len()));
    second_moment.copy_from_slice(&metal_read(&buffers.2, second_moment.len()));
    Ok(())
}
...
pub(crate) fn layer_norm(
    ...
    const SOURCE: &str = r#"
        ...
        kernel void layer_norm(
            ...
            float mean = 0.0f;
            for (uint col = 0; col < width; ++col) mean += values[offset + col];
            mean /= float(width);
            float variance = 0.0f;
            for (uint col = 0; col < width; ++col) {
                float centered = values[offset + col] - mean;
                variance += centered * centered;
            }
            float inverse_std = rsqrt(variance / float(width) + 1.0e-5f);
            for (uint col = 0; col < width; ++col) {
                output[offset + col] = (values[offset + col] - mean) * inverse_std * gamma[col] + beta[col];
            }
        }
    "#;
    let width_u32 = width as u32;
    let (device, queue, pipeline) = metal_pipeline(SOURCE, "layer_norm")?;
    let buffers = (
        metal_input_buffer(&device, values),
        metal_input_buffer(&device, gamma),
        metal_input_buffer(&device, beta),
        metal_output_buffer::<f32>(&device, values.len()),
        metal_input_buffer(&device, &[width_u32]),
    );
    metal_dispatch(
        &queue,
        &pipeline,
    ...
    Ok(metal_read(&buffers.3, values.len()))
}
