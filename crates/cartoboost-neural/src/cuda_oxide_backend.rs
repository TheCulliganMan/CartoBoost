    ...
    pub fn weighted_inverse_scale_mae_loss_f32(
        ...
        if item < 2 {
            ...
            while position < len as usize {
                ...
                if value > 0.0 {
                    total += value * libm::fabsf((prediction[position] - target[position]) * target_scale);
                    weight_total += value;
                }
                position += 1;
            }
            if let Some(value) = loss.get_mut(index) {
                *value = if item == 0 { total / if weight_total == 0.0 { 1.0 } else { weight_total } } else { weight_total };
            }
        }
    }
        ...
        if let Some(value) = gradient.get_mut(index) {
            ...
            if item_weight <= 0.0 || denominator <= 0.0 {
                *value = 0.0;
                ...
                let residual = prediction[item] - target[item];
                let sign = if residual > 0.0 { 1.0 } else if residual < 0.0 { -1.0 } else { 0.0 };
...
impl CudaTensorArena {
    ...
    pub fn affine_f32(
        ...
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to launch cuda-oxide affine: {error}"))
        })
    }
    ...
    pub fn masked_patch_reconstruction_loss_backward_f32_unused(
        ...
        let token_len = checked_product(&[
            ...
            visible.checked_add(masked).ok_or_else(|| {
                NeuralError::InvalidArgument("CUDA masked token count overflows".to_string())
            })?,
            hidden,
        ])?;
    ...
    pub fn batch_norm_channels_parameter_slice_f32(
        ...
        let len = rows.checked_mul(channels).ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA batch-normalization dimensions overflow".to_string())
        })?;
    ...
    pub fn weighted_inverse_scale_mae_loss_backward_f32(
        ...
        if len == 0 || !target_scale.is_finite() || target_scale <= 0.0
            || [prediction, target, weight, prediction_gradient, loss].iter().enumerate()
                .any(|(index, slot)| [prediction, target, weight, prediction_gradient, loss][..index].contains(slot))
            || self.capacity_f32(prediction)? < len || self.capacity_f32(target)? < len || self.capacity_f32(weight)? < len
        {
            return Err(NeuralError::InvalidArgument("invalid CUDA weighted MAE slots or arguments".to_string()));
        }
        self.reserve_f32(prediction_gradient, len)?;
        self.reserve_f32(loss, 2)?;
        let module = kernels::load(&self.context).map_err(|error| NeuralError::InvalidArgument(error.to_string()))?;
        {
            ...
            unsafe {
                module.weighted_inverse_scale_mae_loss_f32(
                    &self.stream, LaunchConfig::for_num_elems(2), prediction_values, target_values,
                    weight_values, loss_values, len as u32, target_scale,
                )
            }.map_err(|error| NeuralError::InvalidArgument(format!("failed to launch CUDA weighted MAE loss: {error}")))?;
        }
        let (prediction_values, target_values, weight_values, loss_values, gradient_values) =
            get_five_f32_slots(&mut self.f32_slots, prediction, target, weight, loss, prediction_gradient)?;
        unsafe {
            module.weighted_inverse_scale_mae_gradient_f32(
                &self.stream, LaunchConfig::for_num_elems(len as u32), prediction_values, target_values,
                weight_values, loss_values, gradient_values, len as u32, target_scale,
            )
        }.map_err(|error| NeuralError::InvalidArgument(format!("failed to launch CUDA weighted MAE gradient: {error}")))
    }
    ...
    pub fn csr_adaptive_logits_parameter_slice_backward_f32(
        ...
        let parameter_len = target_offset.checked_add(nodes * latent).ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA adaptive-logit gradient range overflows".to_string())
        })?;
        if nodes == 0
            || edges == 0
            || latent == 0
        ...
        let indptr_values = self.u32_slots[indptr].as_ref().ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA adaptive-logit indptr is not allocated".to_string())
        })?;
    ...
    pub fn adamw_step_f32(
        ...
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to launch cuda-oxide AdamW: {error}"))
        })
    }
    ...
    pub fn attention_f32(
        ...
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to launch cuda-oxide attention: {error}"))
        })
    }
    ...
    pub fn affine_parameter_slice_f32(
        ...
        let parameter_len = bias_offset.checked_add(output_width).ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA affine parameter range overflows".to_string())
        })?;
        if weights_offset
            .checked_add(weights_len)
            .is_none_or(|end| end > bias_offset)
    ...
    pub fn fill_f32(&mut self, slot: usize, len: usize, value: f32) -> Result<()> {
        ...
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to launch cuda-oxide arena fill: {error}"))
        })
    }
    ...
    pub fn scale_f32(&mut self, input: usize, output: usize, len: usize, scale: f32) -> Result<()> {
        ...
        let source = source.ok_or_else(|| {
            NeuralError::InvalidArgument(format!("CUDA tensor slot {input} has not been allocated"))
        })?;
    ...
    fn launch_relu_like_f32(
        ...
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to launch cuda-oxide {name}: {error}"))
        })
    }
    ...
    pub fn add_tail_time_f32(
        ...
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to launch cuda-oxide tail add: {error}"))
        })
    }
    ...
    pub fn split_channels_f32(
        ...
        let input_len = left_len.checked_add(right_len).ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA channel split dimensions overflow".to_string())
        })?;
        if self.capacity_f32(input)? < input_len {
            return Err(NeuralError::InvalidArgument(
...
impl CudaCsrDiffusionWorkspace {
    pub fn new(indptr: &[u32], indices: &[u32], weights: &[f32]) -> Result<Self> {
        ...
        let indptr_device = DeviceBuffer::from_host(&stream, indptr).map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to upload cuda-oxide CSR indptr: {error}"))
        })?;
...
pub(super) fn dense_layer(
    ...
    .map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to launch cuda-oxide dense layer: {error}"))
    })?;
    output
        .to_host_vec(&stream)
        .map(|values| values.chunks(out_dim).map(ToOwned::to_owned).collect())
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to read cuda-oxide dense output: {error}"))
        })
}
...
pub(super) fn csr_diffusion(
    ...
    let indices = DeviceBuffer::from_host(&stream, indices).map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to upload cuda-oxide CSR indices: {error}"))
    })?;
    ...
    output.to_host_vec(&stream).map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to read cuda-oxide CSR output: {error}"))
    })
}
...
pub(super) fn layer_norm(
    ...
    .map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to launch cuda-oxide layer norm: {error}"))
    })?;
    output.to_host_vec(&stream).map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to read cuda-oxide layer norm: {error}"))
    })
}
...
pub(super) fn pair_sigmoid_scores(
    ...
    let pairs = DeviceBuffer::from_host(&stream, &pairs).map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to upload cuda-oxide pairs: {error}"))
    })?;
    ...
    .map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to launch cuda-oxide pair scores: {error}"))
    })?;
    output
        .to_host_vec(&stream)
        .map(|values| values.into_iter().map(f64::from).collect())
        .map_err(|error| {
            NeuralError::InvalidArgument(format!("failed to read cuda-oxide pair scores: {error}"))
        })
}
...
pub(super) fn csr_row_softmax(indptr: &[u32], logits: &[f32]) -> Result<Vec<f32>> {
    ...
    .map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to launch cuda-oxide CSR softmax: {error}"))
    })?;
    weights.to_host_vec(&stream).map_err(|error| {
        NeuralError::InvalidArgument(format!("failed to read cuda-oxide CSR softmax: {error}"))
    })
}
...
fn checked_product(values: &[usize]) -> Result<usize> {
    values.iter().try_fold(1usize, |product, &value| {
        product.checked_mul(value).ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA tensor dimensions overflow".to_string())
        })
    })
}
...
fn get_three_f32_slots(
    ...
    unsafe {
        let left = (*ptr.add(left)).as_ref().ok_or_else(|| {
            NeuralError::InvalidArgument("CUDA left tensor slot has not been allocated".to_string())
        })?;
...
fn get_four_f32_slots(
    ...
    unsafe {
        let first = (*ptr.add(first)).as_ref().ok_or_else(|| {
            NeuralError::InvalidArgument(format!("CUDA tensor slot {first} has not been allocated"))
        })?;
        ...
        let third = (*ptr.add(third)).as_ref().ok_or_else(|| {
            NeuralError::InvalidArgument(format!("CUDA tensor slot {third} has not been allocated"))
        })?;
        let output = (*ptr.add(output)).as_mut().ok_or_else(|| {
            NeuralError::InvalidArgument(format!(
                "CUDA tensor slot {output} has not been allocated"
...
fn get_five_f32_slots(
    ...
    if indices.iter().any(|index| *index >= slots.len())
        || indices.iter().enumerate().any(|(index, value)| indices[..index].contains(value))
    {
        return Err(NeuralError::InvalidArgument("cuda-oxide arena requires five distinct in-range tensor slots".to_string()));
    }
    let ptr = slots.as_mut_ptr();
    unsafe {
        let get = |index: usize| (*ptr.add(index)).as_ref().ok_or_else(|| NeuralError::InvalidArgument(format!("CUDA tensor slot {index} has not been allocated"));
        let first = get(first)?;
        let second = get(second)?;
        let third = get(third)?;
        let fourth = get(fourth)?;
        let output = (*ptr.add(output)).as_mut().ok_or_else(|| NeuralError::InvalidArgument(format!("CUDA tensor slot {output} has not been allocated")))?;
