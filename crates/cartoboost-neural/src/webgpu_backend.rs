...
fn evaluate_scalar_graph(
    initial: &[f32],
    opcodes: &[u8],
    left: &[u32],
    right: &[u32],
    parameter_ids: Option<&[u32]>,
    parameters: Option<&[f32]>,
) -> Vec<f32> {
    let mut values = initial.to_vec();
    for node in 0..opcodes.len() {
        let lhs = left[node] as usize;
        let rhs = right[node] as usize;
        values[node] = match opcodes[node] {
            0 => initial[node],
            1 => parameter_ids
                .zip(parameters)
                .map_or(initial[node], |(ids, params)| params[ids[node] as usize]),
            2 => values[lhs] + values[rhs],
            3 => values[lhs] * values[rhs],
            4 => values[lhs] / values[rhs].max(1.0e-12),
            5 => values[lhs].tanh(),
            6 => values[lhs].exp(),
            7 => values[lhs].max(1.0e-12).sqrt(),
            8 => values[lhs].sin(),
            9 => 1.0 / (1.0 + (-values[lhs]).exp()),
            10 => values[lhs].max(values[rhs]),
            11 => values[lhs],
            _ => unreachable!("backend validates scalar graph opcodes"),
        };
    }
    values
}
...
pub(crate) fn scalar_graph(
    ...
    Ok(evaluate_scalar_graph(
        initial, opcodes, left, right, None, None,
    ))
}
...
pub(crate) fn scalar_graph_train_step(
    ...
    let values = evaluate_scalar_graph(
        initial,
        opcodes,
        left,
        right,
    ...
    for node in (0..opcodes.len()).rev() {
        ...
        match opcodes[node] {
            ...
            4 => {
                ...
                gradients[rhs] -= gradient * values[lhs] / denominator.powi(2);
            }
            5 => gradients[lhs] += gradient * (1.0 - values[node].powi(2)),
    ...
    let mut parameter_gradients = vec![0.0; parameters.len()];
    for node in 0..opcodes.len() {
        if opcodes[node] == 1 {
            parameter_gradients[parameter_ids[node] as usize] += gradients[node];
        }
    }
    let first_correction = 1.0 - 0.9_f32.powi(step as i32);
    let second_correction = 1.0 - 0.999_f32.powi(step as i32);
    for index in 0..parameters.len() {
        let gradient = parameter_gradients[index] + weight_decay * parameters[index];
        first[index] = 0.9 * first[index] + 0.1 * gradient;
        second[index] = 0.999 * second[index] + 0.001 * gradient * gradient;
        parameters[index] -= learning_rate * (first[index] / first_correction)
            / ((second[index] / second_correction).sqrt() + 1.0e-8);
    }
    Ok(values[loss])
}
