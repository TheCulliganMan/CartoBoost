...
impl HipRuntime {
    ...
    fn compile<T>(
        ...
        let source =
            CString::new(source).map_err(|e| NeuralError::InvalidArgument(e.to_string()))?;
        let name = CString::new("cartoboost_kernel.hip").expect("static kernel name");
        let entry = CString::new(entry).map_err(|e| NeuralError::InvalidArgument(e.to_string()))?;
...
pub(crate) fn dense_layer(
    ...
    let output = run_matrix_kernel("dense", &x, weights, biases, rows, cols, width)?;
