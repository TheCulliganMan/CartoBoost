...
fn as_neural(context: &str, error: windows::core::Error) -> AcceleratorError {
    AcceleratorError::InvalidArgument(format!("DirectML {context} failed: {error}"))
}
...
pub(crate) fn affine_scores(
    ...
    let weights = weights
        ...
        .collect::<Vec<_>>();
    // GEMM bias is indexed by output column, while the accelerator Affine
    // contract provides one intercept per input row. Apply row intercepts
    // after the device matrix product so DirectML matches every other backend.
    directml_gemm(&centered, rows, columns, &weights, 1, &[0.0]).map(|values| {
        values
            .into_iter()
            .zip(intercepts)
            .map(|(value, intercept)| f64::from(value) + intercept)
            .collect()
    })
}
...
pub(crate) fn csr_row_softmax_backward(
    ...
    crate::webgpu_backend::csr_row_softmax_backward(indptr, weights, gradient)
}
pub(crate) fn adamw(
    parameters: &mut [f32],
    first: &mut [f32],
...
pub(crate) fn layer_norm(
    ...
    crate::webgpu_backend::layer_norm(values, rows, width, gamma, beta)
}
pub(crate) fn scalar_graph(
    ...
    crate::webgpu_backend::scalar_graph_f32(values, opcodes, left, right)
}
...
pub(crate) fn scalar_graph_train_step(
    ...
    crate::webgpu_backend::scalar_graph_train_step_f32(
        values,
        opcodes,
        left,
        right,
...
pub(crate) fn train_tanh_mlp(
    ...
    crate::webgpu_backend::train_tanh_mlp(
        inputs,
        targets,
        hidden_size,
        epochs,
...
mod tests {
    ...
    #[test]
    fn affine_scores_apply_distinct_intercept_per_row() {
        let features = vec![vec![1.0, 2.0], vec![3.0, 4.0], vec![-2.0, 0.5]];
        let actual =
            affine_scores(&features, &[0.5, -0.5], &[2.0, -1.0], &[10.0, -3.0, 7.5]).unwrap();
        let expected = features
            .iter()
            .zip([10.0, -3.0, 7.5])
            .map(|(row, intercept)| intercept + (row[0] - 0.5) * 2.0 - (row[1] + 0.5))
            .collect::<Vec<_>>();
