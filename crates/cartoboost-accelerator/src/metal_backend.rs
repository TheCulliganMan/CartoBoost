...
fn metal_pipeline(
    ...
    let device = Device::system_default().ok_or_else(|| {
        AcceleratorError::InvalidArgument("no Metal device is available".to_string())
    })?;
    let library = device
    ...
    let function = library.get_function(kernel, None).map_err(|error| {
        AcceleratorError::InvalidArgument(format!("failed to load Metal kernel {kernel}: {error}"))
    })?;
    let pipeline = device
    ...
    impl MetalScalarContext {
        fn new() -> Result<Self> {
            ...
            let function = library
                ...
                .map_err(|error| AcceleratorError::InvalidArgument(error.to_string()))?;
            let pipeline = device
                .new_compute_pipeline_state_with_function(&function)
                .map_err(|error| AcceleratorError::InvalidArgument(error.to_string()))?;
            let queue = device.new_command_queue();
...
pub(crate) fn metal_vector_add_report(
    ...
    let library = device
        .new_library_with_source(SOURCE, &CompileOptions::new())
        .map_err(|err| {
            AcceleratorError::InvalidArgument(format!("failed to compile Metal kernel: {err}"))
        })?;
    let kernel = library
        .get_function("vector_add_f32", None)
        .map_err(|err| {
            AcceleratorError::InvalidArgument(format!("failed to load Metal kernel: {err}"))
        })?;
    let pipeline = device
        .new_compute_pipeline_state_with_function(&kernel)
        .map_err(|err| {
            AcceleratorError::InvalidArgument(format!("failed to create Metal pipeline: {err}"))
        })?;
