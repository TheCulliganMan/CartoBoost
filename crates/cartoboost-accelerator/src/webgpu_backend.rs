...
static DEVICE_QUEUE: OnceLock<Option<(wgpu::Device, wgpu::Queue)>> = OnceLock::new();
...
async fn request_device_async() -> Result<(wgpu::Device, wgpu::Queue)> {
    ...
    let adapter = instance
        ...
        .await
        .map_err(|err| {
            AcceleratorError::InvalidArgument(format!("failed to request a WebGPU adapter: {err}"))
        })?;
    adapter
        .request_device(&wgpu::DeviceDescriptor::default())
        .await
        .map_err(|err| {
            AcceleratorError::InvalidArgument(format!("failed to request a WebGPU device: {err}"))
        })
}
...
fn request_device() -> Result<(wgpu::Device, wgpu::Queue)> {
    DEVICE_QUEUE
        .get_or_init(|| block_on(request_device_async()).ok())
        .as_ref()
        .cloned()
        .ok_or_else(|| {
            AcceleratorError::InvalidArgument(
                "failed to initialize the shared WebGPU device".to_string(),
            )
        })
}
...
fn webgpu_readback_f32(
    ...
    result.map_err(|err| {
        AcceleratorError::InvalidArgument(format!("failed to map WebGPU readback buffer: {err}"))
    })?;
...
async fn webgpu_readback_f32_async(
    ...
    rx.await
        .map_err(|_| {
            AcceleratorError::InvalidArgument("WebGPU readback channel disconnected".to_string())
        })?
        .map_err(|err| {
            AcceleratorError::InvalidArgument(format!(
                "failed to map WebGPU readback buffer: {err}"
            ))
        })?;
    let data = slice.get_mapped_range();
    let values = unsafe {
...
/// Browser-safe affine scoring implemented by the same WebGPU dense kernel as
/// native backends. Centering is folded into the bias so no host-side pass is
/// needed after upload.
#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn affine_scores_f32_async(
    features: &[Vec<f64>],
    means: &[f64],
    weights: &[f64],
    intercepts: &[f64],
    ...
    if features.is_empty()
        ...
        || weights.len() != means.len() * intercepts.len()
        || features
            .iter()
            .flatten()
            .chain(means)
    ...
    {
        return Err(AcceleratorError::InvalidArgument(
            "affine inputs must be finite, non-empty, and shape-compatible".to_string(),
        ));
    }
    let features_f32 = features
        .iter()
    ...
    let weights_f32 = weights
        ...
        .collect::<Vec<_>>();
    let mut biases = intercepts
        .iter()
        .map(|value| *value as f32)
        .collect::<Vec<_>>();
    for output in 0..intercepts.len() {
        for input in 0..means.len() {
            biases[output] -= means[input] as f32 * weights_f32[input * intercepts.len() + output];
        }
    }
    Ok(dense_layer_f32_async(&features_f32, &weights_f32, &biases)
        .await?
        .into_iter()
        .flatten()
        .map(f64::from)
...
#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn pairwise_squared_distances_f32_async(
    left: &[Vec<f32>],
    right: &[Vec<f32>],
) -> Result<Vec<Vec<f32>>> {
    ...
    {
        return Err(AcceleratorError::InvalidArgument(
            "pairwise-distance inputs must be finite matrices with matching widths".to_string(),
        ));
    }
    ...
    let mut distances = dense_layer_f32_async(left, &weights, &right_norms).await?;
    for (point, row) in left.iter().zip(&mut distances) {
        let norm = point.iter().map(|value| value * value).sum::<f32>();
        for distance in row {
            *distance = (*distance + norm).max(0.0);
        }
    }
    Ok(distances)
}
...
#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn pair_sigmoid_scores_f32_async(
    embeddings: &[Vec<f32>],
    pairs: &[(usize, usize)],
) -> Result<Vec<f64>> {
    if embeddings.is_empty() || embeddings[0].is_empty() {
        return Err(AcceleratorError::InvalidArgument(
            "embeddings must be a non-empty rectangular matrix".to_string(),
        ));
    }
    ...
    if embeddings
        ...
        || pairs
            .iter()
            .any(|(left, right)| *left >= embeddings.len() || *right >= embeddings.len())
    {
        return Err(AcceleratorError::InvalidArgument(
            "pair-scoring inputs are malformed".to_string(),
        ));
    }
    if pairs.is_empty() {
        return Ok(Vec::new());
    ...
    let left = pairs
        .iter()
        .map(|(index, _)| embeddings[*index].clone())
        .collect::<Vec<_>>();
    let mut weights = vec![0.0_f32; dimensions * pairs.len()];
    for (output, (_, index)) in pairs.iter().enumerate() {
        for dimension in 0..dimensions {
            weights[dimension * pairs.len() + output] = embeddings[*index][dimension];
        }
    }
    let scores = dense_layer_f32_async(&left, &weights, &vec![0.0; pairs.len()]).await?;
    Ok(scores
        .iter()
        .enumerate()
        .map(|(index, row)| 1.0 / (1.0 + (-f64::from(row[index])).exp()))
        .collect())
}

/// Applies a CSR diffusion matrix asynchronously on browser WebGPU.
#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn csr_diffusion_f32_async(
    indptr: &[u32],
    indices: &[u32],
    weights: &[f32],
    channels: usize,
    ...
    crate::backend::validate_csr_diffusion_inputs(indptr, indices, weights, channels, values)?;
    const SOURCE: &str = r#"
    ...
    webgpu_readback_f32_async(&device, &queue, &b4, (values.len() * 4) as u64).await
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn csr_diffusion_backward_f32_async(
    indptr: &[u32],
    indices: &[u32],
    weights: &[f32],
    channels: usize,
    ...
    const SOURCE: &str = r#"
        ...
        @compute @workgroup_size(64)fn main(@builtin(global_invocation_id)id:vec3<u32>){
          let i=id.x;
          if(i<params.value_len){let source=(i/params.channels)%params.nodes;let channel=i%params.channels;let batch=i/(params.nodes*params.channels);var sum=0.0;for(var row=0u;row<params.nodes;row++){for(var edge=indptr[row];edge<indptr[row+1u];edge++){if(indices[edge]==source){sum+=weights[edge]*grad[(batch*params.nodes+row)*params.channels+channel];}}}input_grad[i]=sum;}
          if(i<params.edge_len){var row=0u;while(indptr[row+1u]<=i){row++;}let source=indices[i];var sum=0.0;for(var batch=0u;batch<params.batches;batch++){for(var channel=0u;channel<params.channels;channel++){sum+=grad[(batch*params.nodes+row)*params.channels+channel]*values[(batch*params.nodes+source)*params.channels+channel];}}edge_grad[i]=sum;}
        }
    "#;
    let (device, queue) = request_device_async().await?;
    ...
    let b5 = storage_buffer(
        &device,
        &queue,
        "browser-csr-bwd-input",
        &vec![0_u8; values.len() * 4],
        true,
    );
    let b6 = storage_buffer(
        &device,
        &queue,
        "browser-csr-bwd-edge",
        &vec![0_u8; weights.len() * 4],
        true,
    );
    let b7 = storage_buffer(
        &device,
        &queue,
        "browser-csr-bwd-config",
        bytes_of(&config),
        false,
    );
    dispatch_shader(
        ...
        (values.len().max(weights.len()) as u32).div_ceil(64),
    );
    Ok(CsrDiffusionBackward {
        input_grad: webgpu_readback_f32_async(&device, &queue, &b5, (values.len() * 4) as u64)
            .await?,
        edge_grad: webgpu_readback_f32_async(&device, &queue, &b6, (weights.len() * 4) as u64)
            .await?,
    })
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
async fn csr_row_kernel_f32_async(
    indptr: &[u32],
    values: &[f32],
    extra: Option<&[f32]>,
    source: &str,
) -> Result<Vec<f32>> {
    crate::backend::validate_csr_row_inputs(indptr, values)?;
    if let Some(extra) = extra {
        if extra.len() != values.len() || extra.iter().any(|value| !value.is_finite()) {
            return Err(AcceleratorError::InvalidArgument(
                "CSR row gradient must be finite and match the edge shape".to_string(),
            ));
        }
    }
    if values.is_empty() {
    ...
    if let Some(extra) = b2.as_ref() {
        ...
        dispatch_shader(
            ...
            "browser-row-kernel",
            source,
            &[(&b0, true), (&b1, true), (&output, false), (&params, true)],
            (values.len() as u32).div_ceil(64),
        );
    }
    webgpu_readback_f32_async(&device, &queue, &output, (values.len() * 4) as u64).await
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn csr_row_softmax_f32_async(indptr: &[u32], logits: &[f32]) -> Result<Vec<f32>> {
    const SOURCE: &str = r#"
    ...
    "#;
    csr_row_kernel_f32_async(indptr, logits, None, SOURCE).await
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn csr_row_softmax_backward_f32_async(
    indptr: &[u32],
    weights: &[f32],
    output_grad: &[f32],
) -> Result<Vec<f32>> {
    const SOURCE: &str = r#"
        ...
        fn main(@builtin(global_invocation_id) id: vec3<u32>) {
            ...
            var row=0u; while(indptr[row+1u]<=edge){row++;}
            var dot=0.0;
            for(var i=indptr[row];i<indptr[row+1u];i++){dot+=weights[i]*grad[i];}
            output[edge]=weights[edge]*(grad[edge]-dot);
        }
    "#;
    csr_row_kernel_f32_async(indptr, weights, Some(output_grad), SOURCE).await
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn adamw_f32_async(
    parameters: &[f32],
    first_moment: &[f32],
    second_moment: &[f32],
    gradient: &[f32],
    ...
    {
        return Err(AcceleratorError::InvalidArgument(
            "AdamW tensors and optimizer configuration are invalid".to_string(),
        ));
    }
    const SOURCE: &str = r#"
    ...
    let m = storage_buffer(
        &device,
        &queue,
        "browser-adamw-m",
        bytes_of(first_moment),
        true,
    );
    let v = storage_buffer(
        &device,
        &queue,
        "browser-adamw-v",
        bytes_of(second_moment),
        true,
    );
    let g = storage_buffer(
        &device,
        &queue,
        "browser-adamw-g",
        bytes_of(gradient),
    ...
    let config = storage_buffer(
        &device,
        &queue,
        "browser-adamw-config",
        bytes_of(&raw),
        false,
    );
    dispatch_shader(
    ...
    Ok((
        webgpu_readback_f32_async(&device, &queue, &p, (parameters.len() * 4) as u64).await?,
        webgpu_readback_f32_async(&device, &queue, &m, (parameters.len() * 4) as u64).await?,
        webgpu_readback_f32_async(&device, &queue, &v, (parameters.len() * 4) as u64).await?,
    ))
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn layer_norm_f32_async(
    values: &[f32],
    rows: usize,
    width: usize,
    gamma: &[f32],
    ...
    {
        return Err(AcceleratorError::InvalidArgument(
            "layer-normalization tensors are invalid".to_string(),
        ));
    }
    const SOURCE: &str = r#"
    ...
    let x = storage_buffer(&device, &queue, "browser-ln-x", bytes_of(values), false);
    let g = storage_buffer(&device, &queue, "browser-ln-g", bytes_of(gamma), false);
    let b = storage_buffer(&device, &queue, "browser-ln-b", bytes_of(beta), false);
    let output = storage_buffer(
        &device,
        &queue,
        "browser-ln-output",
...
#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn scalar_graph_f32_async(
    initial_values: &[f32],
    opcodes: &[u8],
    left: &[u32],
    right: &[u32],
) -> Result<Vec<f32>> {
    crate::backend::validate_scalar_graph_inputs(initial_values, opcodes, left, right)?;
    const SOURCE: &str = r#"
        struct Params{len:u32,};
        ...
        @compute @workgroup_size(1)fn main(){
          for(var i=0u;i<params.len;i++){
            let base=3u*i;let op=graph[base];
            if(op>1u){let a=values[graph[base+1u]];let b=values[graph[base+2u]];
              switch op{
                case 2u:{values[i]=a+b;}case 3u:{values[i]=a*b;}
                case 4u:{values[i]=a/max(b,1e-12);}case 5u:{values[i]=tanh(a);}
                case 6u:{values[i]=exp(a);}case 7u:{values[i]=sqrt(max(a,1e-12));}
                case 8u:{values[i]=sin(a);}case 9u:{values[i]=1.0/(1.0+exp(-a));}
                case 10u:{values[i]=max(a,b);}case 11u:{values[i]=a;}default:{}
              }
            }
          }
        }
    ...
    let graph = opcodes
        .iter()
        .enumerate()
        .flat_map(|(index, opcode)| [u32::from(*opcode), left[index], right[index]])
        .collect::<Vec<_>>();
    let (device, queue) = request_device_async().await?;
    let values = storage_buffer(
        &device,
        &queue,
        "browser-scalar-values",
        bytes_of(initial_values),
        true,
    );
    let graph = storage_buffer(
        &device,
        &queue,
        "browser-scalar-graph",
        bytes_of(&graph),
        false,
    );
    let config = storage_buffer(
        &device,
        &queue,
        "browser-scalar-config",
        bytes_of(&[initial_values.len() as u32]),
        false,
    );
    dispatch_shader(
        &device,
        ...
        "browser-scalar-graph",
        SOURCE,
        &[(&values, false), (&graph, true), (&config, true)],
        1,
    );
    webgpu_readback_f32_async(&device, &queue, &values, (initial_values.len() * 4) as u64).await
}

#[cfg(all(feature = "webgpu", target_arch = "wasm32"))]
pub async fn train_tanh_mlp_f32_async(
    inputs: &[Vec<f32>],
    targets: &[f32],
    hidden_size: usize,
    epochs: usize,
    ...
    crate::backend::validate_tanh_mlp_training_inputs(
        inputs,
        targets,
        hidden_size,
        epochs,
    ...
    const SOURCE: &str = r#"
        struct Params{rows:u32,input_size:u32,hidden_size:u32,epochs:u32,learning_rate:f32,};
        @group(0)@binding(0)var<storage,read> inputs:array<f32>;
        @group(0)@binding(1)var<storage,read> targets:array<f32>;
        @group(0)@binding(2)var<storage,read_write> p:array<f32>;
        @group(0)@binding(3)var<storage,read> params:Params;
        @compute @workgroup_size(1)fn main(){
          let b1=params.hidden_size*params.input_size;let w2=b1+params.hidden_size;let b2=w2+params.hidden_size;
          for(var epoch=0u;epoch<params.epochs;epoch++){for(var row=0u;row<params.rows;row++){
            var prediction=p[b2];
            for(var h=0u;h<params.hidden_size;h++){var v=p[b1+h];for(var i=0u;i<params.input_size;i++){v+=p[h*params.input_size+i]*inputs[row*params.input_size+i];}prediction+=tanh(v)*p[w2+h];}
            let eg=2.0*(prediction-targets[row]);p[b2]-=params.learning_rate*eg;
            for(var h=0u;h<params.hidden_size;h++){var v=p[b1+h];for(var i=0u;i<params.input_size;i++){v+=p[h*params.input_size+i]*inputs[row*params.input_size+i];}let a=tanh(v);let old=p[w2+h];p[w2+h]-=params.learning_rate*eg*a;let g=eg*old*(1.0-a*a);p[b1+h]-=params.learning_rate*g;for(var i=0u;i<params.input_size;i++){p[h*params.input_size+i]-=params.learning_rate*g*inputs[row*params.input_size+i];}}
          }}
        }
    "#;
    ...
    let y = storage_buffer(
        &device,
        &queue,
        "browser-tanh-targets",
        bytes_of(targets),
        false,
    );
    let p = storage_buffer(
        &device,
        &queue,
        "browser-tanh-parameters",
        bytes_of(parameters),
        true,
    );
    let c = storage_buffer(
        &device,
        &queue,
        "browser-tanh-config",
        bytes_of(&config),
    ...
    dispatch_shader(
        &device,
        &queue,
        "browser-tanh-training",
        SOURCE,
        &[(&x, true), (&y, true), (&p, false), (&c, true)],
        1,
    );
    webgpu_readback_f32_async(&device, &queue, &p, (parameters.len() * 4) as u64).await
}
...
pub async fn scalar_graph_train_step_f32_async(
    initial_values: &[f32],
    opcodes: &[u8],
    left: &[u32],
    right: &[u32],
    ...
    if parameter_ids.len() != opcodes.len()
        || loss >= opcodes.len()
        || parameters.is_empty()
        || first.len() != parameters.len()
        || second.len() != parameters.len()
        || step == 0
        || !learning_rate.is_finite()
        || learning_rate <= 0.0
        || !weight_decay.is_finite()
        || parameter_ids
            .iter()
            .enumerate()
            .any(|(i, p)| opcodes[i] == 1 && (*p as usize) >= parameters.len())
        || parameters
            .iter()
            .chain(first)
            .chain(second)
    ...
    {
        return Err(AcceleratorError::InvalidArgument(
            "scalar-graph training state is invalid".to_string(),
        ));
    }
    const SOURCE: &str = r#"
      struct Config{len:u32,loss:u32,parameter_len:u32,step:u32,learning_rate:f32,weight_decay:f32,};
      @group(0)@binding(0)var<storage,read_write> values:array<f32>;
      @group(0)@binding(1)var<storage,read> graph:array<u32>;
      @group(0)@binding(2)var<storage,read_write> parameters:array<f32>;
      @group(0)@binding(3)var<storage,read_write> first:array<f32>;
      @group(0)@binding(4)var<storage,read_write> second:array<f32>;
      @group(0)@binding(5)var<storage,read_write> gradients:array<f32>;
      @group(0)@binding(6)var<storage,read_write> parameter_gradients:array<f32>;
      @group(0)@binding(7)var<storage,read> config:Config;
      @compute @workgroup_size(1)fn main(){
        for(var i=0u;i<config.len;i++){let base=4u*i;let op=graph[base];if(op==1u){values[i]=parameters[graph[base+3u]];}else if(op>1u){let lhs=graph[base+1u];let rhs=graph[base+2u];let a=values[lhs];let b=values[rhs];switch op{case 2u:{values[i]=a+b;}case 3u:{values[i]=a*b;}case 4u:{values[i]=a/max(b,1e-12);}case 5u:{values[i]=tanh(a);}case 6u:{values[i]=exp(a);}case 7u:{values[i]=sqrt(max(a,1e-12));}case 8u:{values[i]=sin(a);}case 9u:{values[i]=1.0/(1.0+exp(-a));}case 10u:{values[i]=max(a,b);}case 11u:{values[i]=a;}default:{}}}}
        gradients[config.loss]=1.0;var rev=config.len;loop{if(rev==0u){break;}rev-=1u;let base=4u*rev;let op=graph[base];let lhs=graph[base+1u];let rhs=graph[base+2u];let g=gradients[rev];switch op{case 1u:{parameter_gradients[graph[base+3u]]+=g;}case 2u:{gradients[lhs]+=g;gradients[rhs]+=g;}case 3u:{gradients[lhs]+=g*values[rhs];gradients[rhs]+=g*values[lhs];}case 4u:{let d=max(values[rhs],1e-12);gradients[lhs]+=g/d;gradients[rhs]-=g*values[lhs]/(d*d);}case 5u:{gradients[lhs]+=g*(1.0-values[rev]*values[rev]);}case 6u:{gradients[lhs]+=g*values[rev];}case 7u:{gradients[lhs]+=g/(2.0*max(values[rev],1e-12));}case 8u:{gradients[lhs]+=g*cos(values[lhs]);}case 9u:{gradients[lhs]+=g*values[rev]*(1.0-values[rev]);}case 10u:{if(values[lhs]>=values[rhs]){gradients[lhs]+=g;}else{gradients[rhs]+=g;}}case 11u:{gradients[lhs]+=g;}default:{}}}
        let c1=1.0-pow(0.9,f32(config.step));let c2=1.0-pow(0.999,f32(config.step));for(var i=0u;i<config.parameter_len;i++){let g=parameter_gradients[i]+config.weight_decay*parameters[i];first[i]=0.9*first[i]+0.1*g;second[i]=0.999*second[i]+0.001*g*g;parameters[i]-=config.learning_rate*(first[i]/c1)/(sqrt(second[i]/c2)+1e-8);}
      }
    "#;
    let graph = opcodes
        .iter()
        .enumerate()
        .flat_map(|(i, op)| [u32::from(*op), left[i], right[i], parameter_ids[i]])
        .collect::<Vec<_>>();
    let config = [
        opcodes.len() as u32,
        loss as u32,
    ...
    let b2 = storage_buffer(&device, &queue, "browser-sgt-p", bytes_of(parameters), true);
    let b3 = storage_buffer(&device, &queue, "browser-sgt-m", bytes_of(first), true);
    let b4 = storage_buffer(&device, &queue, "browser-sgt-v", bytes_of(second), true);
    let b5 = storage_buffer(
        &device,
        &queue,
        "browser-sgt-g",
        &vec![0_u8; opcodes.len() * 4],
        true,
    );
    let b6 = storage_buffer(
        &device,
    ...
    let values =
        webgpu_readback_f32_async(&device, &queue, &b0, (opcodes.len() * 4) as u64).await?;
    Ok((
        values[loss],
        webgpu_readback_f32_async(&device, &queue, &b2, (parameters.len() * 4) as u64).await?,
        webgpu_readback_f32_async(&device, &queue, &b3, (parameters.len() * 4) as u64).await?,
        webgpu_readback_f32_async(&device, &queue, &b4, (parameters.len() * 4) as u64).await?,
    ))
}
...
#[cfg(all(feature = "webgpu", not(target_arch = "wasm32")))]
#[allow(clippy::too_many_arguments)]
pub(crate) fn scalar_graph_train_step_f32(
    initial_values: &[f32],
    opcodes: &[u8],
    left: &[u32],
    right: &[u32],
    ...
    let graph = opcodes
        ...
        .flat_map(|(i, &op)| [u32::from(op), left[i], right[i], parameter_ids[i]])
        .collect::<Vec<_>>();
    let gradients = vec![0.0f32; opcodes.len()];
    ...
    let b0 = storage_buffer(&device, &queue, "sg-values", bytes_of(initial_values), true);
    let b1 = storage_buffer(&device, &queue, "sg-graph", bytes_of(&graph), false);
    let b2 = storage_buffer(&device, &queue, "sg-p", bytes_of(parameters), true);
    let b3 = storage_buffer(&device, &queue, "sg-m", bytes_of(first), true);
    let b4 = storage_buffer(&device, &queue, "sg-v", bytes_of(second), true);
    let b5 = storage_buffer(&device, &queue, "sg-grad", bytes_of(&gradients), true);
    let b6 = storage_buffer(
        &device,
        &queue,
        "sg-pgrad",
        bytes_of(&parameter_gradients),
        true,
    );
    let b7 = storage_buffer(&device, &queue, "sg-config", bytes_of(&raw), false);
    dispatch_shader(
        &device,
        &queue,
        "webgpu-scalar-train",
    ...
    let values = webgpu_readback_f32(&device, &queue, &b0, (initial_values.len() * 4) as u64)?;
...
pub(crate) fn train_tanh_mlp(
    ...
    const SOURCE: &str = r#"
        ...
        @group(0) @binding(3) var<storage,read> params:Params;
        @compute @workgroup_size(1) fn main() {
            let b1=params.hidden_size*params.input_size;
            let w2=b1+params.hidden_size; let b2=w2+params.hidden_size;
            for(var epoch=0u;epoch<params.epochs;epoch++) { for(var row=0u;row<params.rows;row++) {
                var prediction=p[b2];
                for(var h=0u;h<params.hidden_size;h++) { var v=p[b1+h]; for(var i=0u;i<params.input_size;i++){v+=p[h*params.input_size+i]*inputs[row*params.input_size+i];} prediction+=tanh(v)*p[w2+h]; }
                let eg=2.0*(prediction-targets[row]); p[b2]-=params.learning_rate*eg;
    ...
    let b0 = storage_buffer(&device, &queue, "tanh-mlp-inputs", bytes_of(&flat), false);
    let b1 = storage_buffer(
        &device,
        &queue,
        "tanh-mlp-targets",
    ...
    let b3 = storage_buffer(&device, &queue, "tanh-mlp-config", bytes_of(&raw), false);
    dispatch_shader(
        &device,
        &queue,
        "webgpu-tanh-mlp",
